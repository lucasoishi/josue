...
public class BookControllerImplTest {
    ...
    public void shouldReturnOKWhenListingAnExistingBook() throws Exception {
        ...
        mockMvc.perform(get("/books/" + book.getId()).contentType(MediaType.APPLICATION_JSON)).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.content().json(jsonBook));
    }

    @Test
    public void shouldReturnNotFoundWhenIdDoesNotExistInDB() throws Exception {
        mockMvc.perform(get("/books/" + UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON)).andExpect(MockMvcResultMatchers.status().isNotFound());
    }

    @Test
    public void shouldReturnOKWhenListingBooks() throws Exception {
        mockMvc.perform(get("/books/").contentType(MediaType.APPLICATION_JSON)).andExpect(MockMvcResultMatchers.status().isOk());
    }
    ...
    public void shouldReturnOKWhenCreatingABook() throws Exception {
        ...
        mockMvc.perform(post("/books/").contentType(MediaType.APPLICATION_JSON).content(jsonBook)).andExpect(MockMvcResultMatchers.status().isCreated());
    ...
    public void shouldReturnBadRequestWhenTryingToCreateABookWithMissingRequiredField() throws Exception {
        ...
        mockMvc.perform(post("/books/").contentType(MediaType.APPLICATION_JSON).content(json_payload)).andExpect(MockMvcResultMatchers.status().isBadRequest());
    }
    ...
    public void shouldReturnOkAndCorrectlyUpdateName() throws Exception {
        ...
        mockMvc.perform(patch("/books/" + book.getId()).contentType(MediaType.APPLICATION_JSON).content(payload)).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.content().json(payload));
    ...
    public void shouldReturnNotFoundIfIdDoesNotExistWhenTryingToUpdateName() throws Exception {
        var new_name = "name 2";
        var payload = "{\"name\": \"" + new_name + "\"}";
        mockMvc.perform(patch("/books/" + UUID.randomUUID())
